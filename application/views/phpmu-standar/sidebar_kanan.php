<div class="widget">
	<?php
	  $pasanggambar2 = $this->db->query("SELECT * FROM pasanggambar where judul LIKE '%kanan%'");
	  foreach ($pasanggambar2->result_array() as $b){
		$string = $b['gambar'];
		if ($b['gambar'] != ''){
			if(preg_match("/swf\z/i", $string)) {
				echo "<embed src='".base_url()."asset/foto_pasanggambar/$b[gambar]' width=300 height=240 quality='high' type='application/x-shockwave-flash'>";
			} else {
				echo "<a href='$b[url]' target='_blank'><img style='width:100%' src='".base_url()."asset/foto_pasanggambar/$b[gambar]' alt='$b[judul]' /></a>";
			}
		}
