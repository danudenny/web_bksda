...
class Administrator extends CI_Controller {
    ...
    // Controller Modul Gambar Atas

    function gambaratas(){
        cek_session_akses('gambaratas',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('gambaratas','id_gambaratas','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('gambaratas',array('username'=>$this->session->username),'id_gambaratas','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_gambaratas/view_gambaratas',$data);
    }

    function tambah_gambaratas(){
        cek_session_akses('gambaratas',$this->session->id_session);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_gambaratas/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('gambaratas',$data);  
            redirect($this->uri->segment(1).'/gambaratas');
        }else{
            $this->template->load('administrator/template','administrator/mod_gambaratas/view_gambaratas_tambah');
        }
    }

    function edit_gambaratas(){
        cek_session_akses('gambaratas',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_gambaratas/';
            ...
            $where = array('id_gambaratas' => $this->input->post('id'));
            $this->model_app->update('gambaratas', $data, $where);
            redirect($this->uri->segment(1).'/gambaratas');
        }else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('gambaratas', array('id_gambaratas' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('gambaratas', array('id_gambaratas' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_gambaratas/view_gambaratas_edit',$data);
        }
    }

    function delete_gambaratas(){
        cek_session_akses('gambaratas',$this->session->id_session);
        if ($this->session->level=='admin'){
            $id = array('id_gambaratas' => $this->uri->segment(3));
        }else{
            $id = array('id_gambaratas' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('gambaratas',$id);
        redirect($this->uri->segment(1).'/gambaratas');
    }


	// Controller Modul Gambar Home

	function gambarhome(){
		cek_session_akses('gambarhome',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('gambartengah','id_gambartengah','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('gambartengah',array('username'=>$this->session->username),'id_gambartengah','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_gambarhome/view_gambarhome',$data);
	}

	function tambah_gambarhome(){
		cek_session_akses('gambarhome',$this->session->id_session);
		if (isset($_POST['submit'])){
    		$config['upload_path'] = 'asset/foto_gambartengah/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('gambartengah',$data);  
			redirect($this->uri->segment(1).'/gambarhome');
		}else{
			$this->template->load('administrator/template','administrator/mod_gambarhome/view_gambarhome_tambah');
		}
	}

	function edit_gambarhome(){
		cek_session_akses('gambarhome',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			$config['upload_path'] = 'asset/foto_gambartengah/';
            ...
            $where = array('id_gambartengah' => $this->input->post('id'));
            $this->model_app->update('gambartengah', $data, $where);
			redirect($this->uri->segment(1).'/gambarhome');
		}else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('gambartengah', array('id_gambartengah' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('gambartengah', array('id_gambartengah' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_gambarhome/view_gambarhome_edit',$data);
		}
	}

	function delete_gambarhome(){
        cek_session_akses('gambarhome',$this->session->id_session);
		if ($this->session->level=='admin'){
            $id = array('id_gambartengah' => $this->uri->segment(3));
        }else{
            $id = array('id_gambartengah' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('gambartengah',$id);
		redirect($this->uri->segment(1).'/gambarhome');
	}


    // Controller Modul Gambar Sidebar

    function gambarsidebar(){
        cek_session_akses('gambarsidebar',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('pasanggambar','id_pasanggambar','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('pasanggambar',array('username'=>$this->session->username),'id_pasanggambar','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_gambarsidebar/view_gambarsidebar',$data);
    }

    function tambah_gambarsidebar(){
        cek_session_akses('gambarsidebar',$this->session->id_session);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_pasanggambar/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '3000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('c');
            ...
            $this->model_app->insert('pasanggambar',$data);
            redirect($this->uri->segment(1).'/gambarsidebar');
        }else{
            $this->template->load('administrator/template','administrator/mod_gambarsidebar/view_gambarsidebar_tambah');
        }
    }

    function edit_gambarsidebar(){
        cek_session_akses('gambarsidebar',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_pasanggambar/';
            ...
            $where = array('id_pasanggambar' => $this->input->post('id'));
            $this->model_app->update('pasanggambar', $data, $where);
            redirect($this->uri->segment(1).'/gambarsidebar');
        }else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('pasanggambar', array('id_pasanggambar' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('pasanggambar', array('id_pasanggambar' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_gambarsidebar/view_gambarsidebar_edit',$data);
        }
    }

    function delete_gambarsidebar(){
        cek_session_akses('gambarsidebar',$this->session->id_session);
        if ($this->session->level=='admin'){
            $id = array('id_pasanggambar' => $this->uri->segment(3));
        }else{
            $id = array('id_pasanggambar' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
        $this->model_app->delete('pasanggambar',$id);
        redirect($this->uri->segment(1).'/gambarsidebar');
    }
    ...
    // Controller Modul Link SKPD

    function linkskpd(){
        cek_session_akses('linkskpd',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('link_skpd','id','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('link_skpd','id','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_linkskpd/view_linkskpd',$data);
    }

    function tambah_linkskpd(){
        cek_session_akses('linkskpd',$this->session->id_session);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/link_skpd/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|swf';
            $config['max_size'] = '5000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('b');
			...
			$this->model_app->insert('link_skpd',$data);
			redirect($this->uri->segment(1).'/linkskpd');
		}else{
			$this->template->load('administrator/template','administrator/mod_linkskpd/view_linkskpd_tambah');
		}

    }

    function edit_linkskpd(){
        cek_session_akses('linkskpd',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/link_skpd/';
			...
			$this->model_app->update('link_skpd', $data, $where);
			redirect($this->uri->segment(1).'/linkskpd');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('link_skpd', array('id' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('link_skpd', array('id' => $id))->row_array();
            }
			$data = array('rows' => $proses);
			$this->template->load('administrator/template','administrator/mod_linkskpd/view_linkskpd_edit',$data);
		}
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $data = array('nama'=>$this->input->post('a'),
                          'gambar'=>$this->input->post('b'),
    ...
    function delete_linkskpd(){
        ...
        $this->model_app->delete('link_skpd',$id);
        redirect($this->uri->segment(1).'/linkskpd');
    }
    ...
    function upgrade(){
        ...
        $this->db->query("ALTER TABLE `gambaratas` ADD COLUMN IF NOT EXISTS `source` TEXT NOT NULL AFTER `url`");
        $this->db->query("ALTER TABLE `gambartengah` ADD COLUMN IF NOT EXISTS `source` TEXT NOT NULL AFTER `url`");
        $this->db->query("ALTER TABLE `gambartengah` ADD COLUMN IF NOT EXISTS `posisi` VARCHAR(50) NOT NULL DEFAULT 'home' AFTER `tgl_posting`");
        $this->db->query("ALTER TABLE `kategori` ADD COLUMN IF NOT EXISTS `gambar_utama` TEXT NOT NULL AFTER `sidebar`");
        $this->db->query("ALTER TABLE `pasanggambar` ADD COLUMN IF NOT EXISTS `source` TEXT NOT NULL AFTER `url`");
